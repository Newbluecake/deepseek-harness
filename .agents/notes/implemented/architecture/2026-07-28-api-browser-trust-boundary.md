...
Reachability stays the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`). This fence remains a confused-deputy defense, not an authentication layer; authentication is now owned separately by [the Web password-login admission decision](2026-08-22-web-password-login-admission-seat.md). A live, origin-bound session may satisfy this fence's Host check without a `trustedHosts` entry, while the Origin and cross-site checks remain unchanged. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and either `trustedHosts` or a session establishing the serving authority, the socket address adds nothing the header fence does not already cover.
...
- **Authentication inside this trust-fence change.** Rejected: token minting, storage, and rotation were separate product surface, and the fence closed the browser-deputy holes without pre-deciding that design. Authentication later landed as a separate whole-site admission plugin, preserving this separation of responsibilities.

## Consequences

- Any future `/api` method is covered by construction; there is no per-route trust decision left to forget.
- An unauthenticated non-loopback deployment must have its serving authorities trusted or requests are refused. The dsh CLI derives machine LAN IP literals into the connection row (port-less entries — an IP-literal Host cannot be a rebound name, and the bound port may be OS-assigned) and offers `dsh web --trusted-host` for named authorities; compositions the CLI does not boot declare `trustedHosts` themselves. An authenticated deployment instead lets an origin-bound session satisfy the Host check and needs no authority list. Non-browser automation without such a session rides the original fence: loopback, a derived LAN IP, or a declared authority passes; an undeclared DNS alias is refused.
- Clients must label POST bodies `application/json` (ours always did; raw-fetch tests gained the header).
- A non-loopback bind with no admission owner is unavailable: the webserver answers 503 rather than publishing an unauthenticated dispatch to the network. `trustedHosts` stays the rebinding fence for whatever unauthenticated requests an admission policy admits.
