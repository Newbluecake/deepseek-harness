...
可达性仍由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制。这道栅栏依然是混淆代理人防御，不是认证层；认证现在由 [Web 口令登录准入决策](2026-08-22-web-password-login-admission-seat.zh.md)单独拥有。存活且与源绑定的会话可以无需 `trustedHosts` 条目而满足这道栅栏的 Host 检查，Origin 与跨站检查则保持不变。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性，再由 `trustedHosts` 或会话建立服务权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- **在本次信任栅栏变更里加入认证。** 否决：令牌的签发、存储与轮换是独立的产品面，而栅栏无需预先决定认证设计即可封死浏览器混淆代理人漏洞。认证随后作为独立的整站准入插件落地，保留了这项职责分离。

## 后果

- 未来任何 `/api` 方法天然在覆盖范围内；不存在会被遗忘的按路由信任决定。
- 未认证的非回环部署必须把对外服务 authority 列入信任范围，否则请求会被拒绝。dsh CLI 把本机 LAN IP 字面量推导进 connection 行（不带端口的条目——IP 字面量 Host 不可能是被重绑的域名，且绑定端口可能由操作系统分配），并提供 `dsh web --trusted-host` 声明具名权威；并非由 CLI 启动的组合自行声明 `trustedHosts`。已认证部署则由与源绑定的会话满足 Host 检查，无需权威列表。没有此类会话的非浏览器自动化仍走原有栅栏：回环地址、推导的 LAN IP 或已声明的权威可通过；未声明的 DNS 别名会被拒绝。
- 客户端必须给 POST 体标注 `application/json`（我们自己的客户端一向如此；裸 fetch 测试补上了该头）。
- 没有准入归属者的非回环绑定不可用：webserver 回 503，而不是把未认证的分发发布到网络上。`trustedHosts` 仍是 rebinding 栅栏，覆盖准入策略放行的那些未认证请求。
