...
- **Host 栅栏无法区分两个已认证的源** — 单个会话即可满足本部署所服务的全部权威，因此已认证部署在跨站标记之外，并未获得任何按权威区分的隔离。
