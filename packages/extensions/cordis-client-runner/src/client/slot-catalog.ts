...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-file-explorer FileExplorer',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'file-explorer.overlay',
    kind: 'list',
    scope: 'session',
    summary: 'The file-explorer modal layer: text preview, selected-file git diff, and Git Tree entries, rendered from inside the `details` panel (fixed positioning lifts them above the frame).',
    doc: 'The file-explorer modal layer: text preview, selected-file git diff,\nand Git Tree entries, rendered from inside the `details` panel (fixed\npositioning lifts them above the frame). Session scope so the modals\nshare the tree\'s viewing-store handle.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'details\' (client-ui-file-explorer), so it exists while that entry is mounted',
    occupants: [
      'client-ui-file-explorer CodeViewer id \'file-explorer-viewer\'',
      'client-ui-file-explorer GitModal id \'file-explorer-git\'',
      'client-ui-file-explorer GitTreeModal id \'file-explorer-git-tree\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'file-explorer.overlay\', () => ctx.slots.register(\n      { name: \'file-explorer.overlay\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-file-explorer/src/client/contract/slots.ts:47',
  },
  {
    key: 'root',
    kind: 'single',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-file-explorer FileExplorerRail id \'file-explorer-rail\'',
      'client-ui-terminal TerminalPanel id \'terminal-panel\'',
    ],
    replaceRisk: 'none',
